...
def evaluate_classification(model, name, X_train, X_test, y_train, y_test):
    ...
    train_precision = metrics.precision_score(
        y_train, model.predict(X_train), average="weighted", zero_division=1
    )
    test_precision = metrics.precision_score(
        y_test, model.predict(X_test), average="weighted", zero_division=1
    )
...
def train_and_save_model(num_rows=None):
    ...
    # Load the dataset
    df = pd.read_csv(
        "/Users/shekhargoud/Desktop/creditcard_prediction/dataset/creditcard_2023.csv"
    )
    ...
    # If num_rows is specified, use a subset of the data
    if num_rows:
    ...
    print("Applying label encoding on the target column...")
    ...
    pipeline = Pipeline(
        [
            ("preprocessor", StandardScaler()),  # StandardScaler for feature scaling
            (
                "classifier",
                XGBClassifier(
...
if __name__ == "__main__":
    # Set the number of rows for training (None for the full dataset)
    num_rows = None
